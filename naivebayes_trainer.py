...
def prob_compute(feature_counts_table, total_counts_table):
    prob_table = {}
    for feature, counts in feature_counts_table.items():
        prob_table[feature] = {}
        for label, count in counts.items():
            prob_table[feature][label] = count / total_counts_table[label]
    return prob_table
   



with open('/home/thalavai-manikandan/Desktop/task 1/task1/dataset(1).json', 'r') as f:
    ds = json.load(f)
    ...
    for each_feature in features:
        ...
        probability_table[each_feature]= prob_compute(fx_counts[each_feature], tx_counts[each_feature])  
    print(probability_table)
    pickle.dump(probability_table, open('probability_table.pkl', 'wb'))
    pickle.dump(yes_no_probs, open('yes_no_probs.pkl', 'wb'))       
#print(feature_counts)
#print(yes_counts, no_counts,yes_no_probs) 
#print(prob_compute(fx_counts['Temperature'], tx_counts['Temperature']))
#print(prob_compute(fx_counts['Outlook'], tx_counts['Outlook']))
#print(prob_compute(fx_counts['Humidity'], tx_counts['Humidity']))
#print(prob_compute(fx_counts['Windy'], tx_counts['Windy']))
#print(yes_no_probs)